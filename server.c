 ...
 * client_id: ID của client gửi thông điệp
 * username: Tên đăng nhập
 * password: Mật khẩu
 * data: Dữ liệu bổ sung (thông báo lỗi, thông tin phản hồi)
 */
typedef struct {
    int type;
    uint32_t client_id;
            ...
            case MSG_REGISTER: {
                ...
                if (register_user(msg.username, msg.password)) {
                    ...
                    sprintf(log_msg, "REGISTER: New user '%s' registered successfully | IP: %s | Time: %s | Total users: %d", 
                           msg.username, client_ip, time_str, user_count);
                    write_log(log_msg);
                } else {
                    response.type = MSG_ERROR;
                    strcpy(response.data, "Registration failed - Username already exists");
                }
                break;
            }
            
            case MSG_LOGIN: {
                printf("Login request from %s (IP: %s)\n", msg.username, client_ip);
                
                int user_idx = authenticate_user(msg.username, msg.password);
                if (user_idx >= 0) {
                    // Cập nhật thông tin phiên đăng nhập
                    pthread_mutex_lock(&users_mutex);
                    users[user_idx].is_online = 1;
                    users[user_idx].client_id = msg.client_id;
                    strcpy(users[user_idx].ip_address, client_ip);
                    pthread_mutex_unlock(&users_mutex);
                    
                    response.type = MSG_SUCCESS;
                    response.client_id = msg.client_id;
                    strcpy(response.data, "Login successful - Đăng nhập thành công");
                    ...
                    char *time_str = ctime(&now);
                    time_str[strlen(time_str) - 1] = '\0';
                    sprintf(log_msg, "LOGIN: User '%s' logged in successfully | Client ID: %u | IP: %s | Time: %s", 
                           msg.username, msg.client_id, client_ip, time_str);
                    write_log(log_msg);
                } else {
                    response.type = MSG_ERROR;
                    strcpy(response.data, "Login failed - Invalid username or password");
                }
                break;
            }
            
            case MSG_LOGOUT: {
                printf("Logout request from client ID %u (IP: %s)\n", msg.client_id, client_ip);
                
                pthread_mutex_lock(&users_mutex);
                int user_idx = find_user_by_id(msg.client_id);
                if (user_idx >= 0) {
                    ...
                    sprintf(log_msg, "LOGOUT: User '%s' logged out | Client ID: %u | IP: %s | Time: %s", 
                           users[user_idx].username, msg.client_id, client_ip, time_str);
                    write_log(log_msg);
                }
                pthread_mutex_unlock(&users_mutex);
                
                response.type = MSG_SUCCESS;
                strcpy(response.data, "Logout successful - Đăng xuất thành công");
                break;
            }
            
            default:
                response.type = MSG_ERROR;
                strcpy(response.data, "Unknown message type - Loại thông điệp không xác định");
                ...
                char *time_str = ctime(&now);
                time_str[strlen(time_str) - 1] = '\0';
                sprintf(error_log, "ERROR: Unknown message type %d | IP: %s | Time: %s", 
                       msg.type, client_ip, time_str);
                write_log(error_log);
                break;
        }
